...
def setup(args, lr, wd, final_runs, run_idx=None, seed=None):
    ...
    if final_runs == 'init_train':
        ...
        if 'P_VK' in cfg.MODEL.TRANSFER_TYPE:
            ...
            pass
            # files = "/home/ch7858/rz/vpt/dumped/vis_fpt/fft_cifar100_forVPT_10_1-0_ablation/vtab-cifar(num_classes=100)/sup_vitb16_224/lr6.25_wd0.001/run1/eval_results.pth"
            # lr, wd = find_best_lrwd(files, cfg.DATA.NAME)
...
def cal_attribution_score(cfg, args, final_runs, model_path):
    # clear up residual cache from previous runs
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    # main training / eval actions here
    ...
    if test_loader:
        ...
        if cfg.ATTRIBUTION_TYPE == "specific":
            ...
            trainer.eval_classifier_GENERAL(model, train_loader, test_loader, prefix="test", integrated_method=cfg.ATTRIBUTION_INTEGRATED_METHOD, model_path=model_path)
...
def main(args):
    ...
    cfg = setup(args, 2.5, 0.001, final_runs='attribution')
    # train(cfg, args, final_runs=False) # originally True here.
    mp = "./last_model.pth"
    print("here1")
    cal_attribution_score(cfg, args, final_runs=True, model_path=mp)
