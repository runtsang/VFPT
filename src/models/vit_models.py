...
class ViT(nn.Module):
    ...
    def __init__(self, cfg, load_pretrain=True, vis=False):
        ...
        if cfg.MODEL.TRANSFER_TYPE != "end2end" and "prompt" not in cfg.MODEL.TRANSFER_TYPE and "prompt_with_attention_sink" not in cfg.MODEL.TRANSFER_TYPE and "QKV" not in cfg.MODEL.TRANSFER_TYPE and "P_VK" not in cfg.MODEL.TRANSFER_TYPE:
            # linear, cls, tiny-tl, parital, adapter
            self.froze_enc = True
        else:
            print('1. should pass here!')
            # prompt, prompt_with_attention_sink, end2end, cls+prompt
        ...
        if 'P_with_attention_sink' in cfg.MODEL.TRANSFER_TYPE:
            print('2. go through vit_models.py -- P_with_attention_sink')
            prompt_attention_sink_cfg = cfg.MODEL.PROMPT_AS
        else:
            prompt_attention_sink_cfg = None
        
        if 'fpt' in cfg.MODEL.TRANSFER_TYPE:
            print('2. go through vit_models.py -- fpt')
            prompt_fourier_cfg = cfg.MODEL.PROMPT_FOURIER
        else:
            prompt_fourier_cfg = None

        self.build_backbone(
            prompt_cfg, prompt_attention_sink_cfg, prompt_fourier_cfg, cfg, adapter_cfg, qkv_cfg, p_vk_cfg, load_pretrain, vis=vis)
        self.cfg = cfg
        self.setup_side()
        self.setup_head(cfg)
    ...
    def build_backbone(self, prompt_cfg, prompt_attention_sink_cfg, prompt_fourier_cfg, cfg, adapter_cfg, qkv_cfg, p_vk_cfg, load_pretrain, vis):
        transfer_type = cfg.MODEL.TRANSFER_TYPE
        self.enc, self.feat_dim = build_vit_sup_models(
            cfg.DATA.FEATURE, cfg.DATA.CROPSIZE, prompt_cfg, prompt_attention_sink_cfg, prompt_fourier_cfg, cfg.MODEL.MODEL_ROOT, adapter_cfg, load_pretrain, vis, qkv_cfg, p_vk_cfg
        )
        ...
        elif transfer_type == "P_with_attention_sink":
            for k, p in self.enc.named_parameters():
                print('#####', k)
                if not prompt_attention_sink_cfg.FT_PT_MIXED: # if not mixed, then freeze all except prompt
                    if "prompt_with_attention_sink" not in k and 'prompt' not in k:
                        p.requires_grad = False
                        print(p.requires_grad) # prompt and deep_prompt both True
        
        elif transfer_type == "fpt":
            for k, p in self.enc.named_parameters():
                if not prompt_fourier_cfg.FT_PT_MIXED: # if not mixed, then freeze all except prompt
                    if "fpt" not in k and 'prompt' not in k:
                        if 'FT' in k:
                            if prompt_fourier_cfg.FOURIER_TYPE == "fixed_linear":
                                p.requires_grad = False
                        else:
                            p.requires_grad = False
...
class Swin(ViT):
    ...
    def __init__(self, cfg):
        super(Swin, self).__init__(cfg, vis=False)
    
    def build_backbone(self, prompt_cfg, prompt_attention_sink_cfg, prompt_fourier_cfg, cfg, adapter_cfg, qkv_cfg, p_vk_cfg, load_pretrain, vis):
        transfer_type = cfg.MODEL.TRANSFER_TYPE
        self.enc, self.feat_dim = build_swin_model(
            cfg.DATA.FEATURE, cfg.DATA.CROPSIZE,
            prompt_cfg, p_vk_cfg, cfg.MODEL.MODEL_ROOT, prompt_fourier_cfg=prompt_fourier_cfg
        )
        ...
        if transfer_type == "partial-1":
            ...
            print('x. go through swin_models.py -- fpt swin')
            for k, p in self.enc.named_parameters():
                if not prompt_fourier_cfg.FT_PT_MIXED: # if not mixed, then freeze all except prompt
                    if "fpt" not in k and 'prompt' not in k:
                        if 'FT' in k:
                            ...
                            p.requires_grad = False
                if p.requires_grad == True or "FT" in k:
                    print('#####', k)
                    print(p.requires_grad) # prompt and deep_prompt both True
...
class SSLViT(ViT):
    ...
    def __init__(self, cfg):
        super(SSLViT, self).__init__(cfg, vis=False)
    # self, prompt_cfg, cfg, adapter_cfg, qkv_cfg, p_vk_cfg, load_pretrain, vis
    # self, prompt_cfg, cfg, adapter_cfg, load_pretrain, vis
    # self, prompt_cfg, prompt_attention_sink_cfg, prompt_fourier_cfg, cfg, adapter_cfg, qkv_cfg, p_vk_cfg, load_pretrain, vis
    def build_backbone(self, prompt_cfg, prompt_attention_sink_cfg, prompt_fourier_cfg, cfg, adapter_cfg, qkv_cfg, p_vk_cfg, load_pretrain, vis):
        ...
        self.enc, self.feat_dim = build_fn(
            cfg.DATA.FEATURE, cfg.DATA.CROPSIZE,
            prompt_cfg, p_vk_cfg, cfg.MODEL.MODEL_ROOT, prompt_fourier_cfg=prompt_fourier_cfg, adapter_cfg=adapter_cfg
        )
        ...
        if transfer_type == "partial-1":
            ...
            print('x. go through vit_models.py -- fpt ssl')
            for k, p in self.enc.named_parameters():
                if not prompt_fourier_cfg.FT_PT_MIXED: # if not mixed, then freeze all except prompt
                    if "fpt" not in k and 'prompt' not in k:
                        if 'FT' in k:
