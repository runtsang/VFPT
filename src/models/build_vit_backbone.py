...
from .vit_prompt.vit_wo_pe import PromptedVisionTransformer as PromptedVisionTransformer2
from .vit_prompt.vit_attention_sink import PromptedVisionTransformer_AttentionSink
from .vit_prompt.vit_fourier import PromptedVisionTransformer_Fourier
from .vit_prompt.vit_fourier_wo_pe import PromptedVisionTransformer_Fourier as PromptedVisionTransformer_Fourier2
...
from .vit_prompt.vit_fourier_moco import vit_base as prompt_vit_base_fourier
from .vit_prompt.vit_fourier_mae import build_model as prompt_mae_vit_model_fourier
from .vit_prompt.swin_transformer_fourier import PromptedSwinTransformer_fourier
...
def build_mae_model(
    model_type, crop_size, prompt_cfg, p_vk_cfg, model_root, adapter_cfg=None, prompt_fourier_cfg=None
): # new added p_vk_cfg
    if prompt_fourier_cfg is not None:
        print('3. Go through prompt_fourier_cfg --- build_mae_model.py') 
        model = prompt_mae_vit_model_fourier(model_type, prompt_fourier_cfg)
...
def build_mocov3_model(
    model_type, crop_size, prompt_cfg, p_vk_cfg, model_root,  prompt_fourier_cfg=None, adapter_cfg=None
):
    if model_type != "mocov3_vitb":
        raise ValueError("Does not support other arch")
    if prompt_fourier_cfg is not None:
        print('3. Go through prompt_fourier_cfg --- build_mocov3_model.py') 
        model = prompt_vit_base_fourier(prompt_fourier_cfg)
    elif prompt_cfg is not None:
...
def build_swin_model(model_type, crop_size, prompt_cfg, p_vk_cfg, model_root, prompt_fourier_cfg=None):
    if prompt_fourier_cfg is not None:
        return _build_prompted_swin_model_fourier(
            model_type, crop_size, prompt_cfg, p_vk_cfg, model_root, prompt_fourier_cfg=prompt_fourier_cfg)
    elif prompt_cfg is not None:
...
def _build_prompted_swin_model_fourier(model_type, crop_size, prompt_cfg, p_vk_cfg, model_root, prompt_fourier_cfg):
    if model_type == "swint_imagenet":
        model = PromptedSwinTransformer_fourier(
            prompt_fourier_cfg,
            img_size=crop_size,
            embed_dim=96,
            depths=[2, 2, 6, 2],
...
def build_vit_sup_models(
    model_type, crop_size, prompt_cfg=None, prompt_attention_sink_cfg=None, prompt_fourier_cfg=None, model_root=None, adapter_cfg=None, load_pretrain=True, vis=False, qkv_cfg=None, p_vk_cfg=None
):
    ...
    if prompt_cfg is not None:
        ...
        if prompt_cfg.POSITION_EMBEDDINGS == True:
            ...
            print('3. Go through Origin Prompt without position embeddings --- build_vit_backbone.py')
            model = PromptedVisionTransformer2(
                prompt_cfg, model_type,
                crop_size, num_classes=-1, vis=vis
            )
    
    elif prompt_attention_sink_cfg is not None:
        # For further changes in [cls] token
        print('3. Go through Prompt with attention sink --- build_vit_backbone.py')
        model = PromptedVisionTransformer_AttentionSink(
            prompt_attention_sink_cfg, model_type,
            crop_size, num_classes=-1, vis=vis
        )
        
    elif prompt_fourier_cfg is not None:
        # For further changes in [cls] token
        if prompt_fourier_cfg.POSITION_EMBEDDINGS == True:
            print('3. Go through Prompt with fourier --- build_vit_backbone.py')
            model = PromptedVisionTransformer_Fourier(
                prompt_fourier_cfg, model_type,
                crop_size, num_classes=-1, vis=vis
            )
        else:
            print('3. Go through Prompt with fourier without position embeddings --- build_vit_backbone.py')
            model = PromptedVisionTransformer_Fourier2(
                prompt_fourier_cfg, model_type,
                crop_size, num_classes=-1, vis=vis
            )
