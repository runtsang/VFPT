...
def _construct_loader(cfg, split, batch_size, shuffle, drop_last):
    ...
    if dataset_name.startswith("vtab-"):
        ...
        if cfg.DATA.FOURIER == False:
            from .datasets.tf_dataset import TFDataset
            dataset = TFDataset(cfg, split)
        else:
            from .datasets.tf_dataset import TFDataset_Fourier
            dataset = TFDataset_Fourier(cfg, split, cfg.DATA.FOURIER_PATH, cfg.DATA.FOURIER_LB)
    else:
        assert (
