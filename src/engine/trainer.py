...
class Trainer():
    ...
    @torch.no_grad()
    def eval_classifier_GENERAL(self, model, train_loader, data_loader, prefix, integrated_method, model_path):
        ...
        for idx, input_data in enumerate(data_loader):
            ...
            if attribution_patches is not None:
                ...
                if integrated_method == 'ig':
                    ...
                    print(f'######./attribution_images_saved/{self.cfg.MODEL.TRANSFER_TYPE}/pytorch_gradcam')
                    ...
                    for i in range(attribution_patches.shape[0]):
                        # unique_id = str(uuid.uuid4())
                        unique_id = next(id_iter)
                        filename = f'./attribution_images_saved/{self.cfg.MODEL.TRANSFER_TYPE}/pytorch_gradcam/pytorch_gradcam_{unique_id}_{targets[i]}_{torch.argmax(outputs[i])}.png'
